...
- **UG BB-markup as styling hints, not pure noise** (TODO 2026-05-28): today `[ch]X[/ch]` and `[tab]…[/tab]` are stripped in both pipelines (crawler/build-private-bundle.py and on-device addLocalImport via catalog.js's cleanUgBody) — lossless visually, but we throw away semantic information UG explicitly gave us: "this token is a chord", "this block is chord-over-lyric". Wiring them into rendering would let us style chord tokens (e.g. subtle color / weight so chord letters pop on a monospace background) and treat tab blocks as a structural unit (e.g. `<pre class="ug-block">` wrappers that could opt into different chord-wrap behaviour or alignment hints). Strip remains the default fallback for anything else. Probably a `parseUgBody()` helper that returns a tokenized structure + CSS hooks in style.css. Worth doing after we've decided what the chord-color story is more broadly (the existing `[ch]` view is plain text, so it'd be visible across catalog tabs too if we change tab-body rendering uniformly).
