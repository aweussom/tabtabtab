...
- **Chord-token styling in rendered tab bodies** (✅ shipped 2026-05-30, this commit's predecessor): chord letters now render in a warm rust tint + semibold (`.tab-body .chord` in style.css) on detected chord lines. Implementation went via **heuristic detection** rather than the originally-sketched BB-markup-based parsing: `decorateChordTokens` in `views/tab.js` re-uses `chord-wrap.js`'s `isChordLine` + `CHORD_TOKEN_RE` to identify chord lines after every `wrapTabBody` pass, then wraps matching tokens in `<span class="chord">`. Works uniformly across UG-imports (where `[ch]X[/ch]` markers are still stripped on import) and nortabs catalog (where `chopro_chord` HTML wrappers are still stripped via `cleanTabBody`). The heuristic threshold (≥70% chord-shaped tokens) misses chord lines with embedded commentary in a single line — extremely rare; revisit with explicit BB-markup parsing if it comes up.
