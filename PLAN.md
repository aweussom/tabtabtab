...
- **Randomized small UG-test-sample generator** (TODO 2026-05-28): a small Python script that reads `crawler/private/ug-import.json` (or any `nortabs-ug-import-*.json`), samples a random subset of 12-20 tabs, and writes it out as a valid UG-export-shaped JSON. Speeds up testing the `#/import/ug` flow dramatically — full 253-tab enrichment is ~40 min on-device per test cycle; a 12-20-tab sample is ~2-3 min. Random sampling beats always-taking-the-first-N (which biases toward whatever order the Tampermonkey export produced, usually alphabetical). Output e.g. to `crawler/private/ug-import-sample-12.json`. Probably ~15 lines: `argparse`, `json.load`, `random.sample`, `json.dump`. Gitignore the sample output (it's derivative + per-run).
