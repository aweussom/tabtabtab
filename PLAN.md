...
> **For a focused open-only backlog without all the shipped-item detail, see [TODO.md](TODO.md). For the explicit non-features list ("things we've decided NOT to do"), see [TODONT.md](TODONT.md). This section keeps the full design log including detailed shipped-item rationales for reference.**


- **Chord-token styling in rendered tab bodies** (✅ shipped 2026-05-30, `17d8db3` for chord-over-lyric lines + `<next-commit>` for ChordPro inline + chopro_tab strip): chord letters render in a warm rust tint + semibold (`.tab-body .chord` in style.css). `decorateChordTokens` in `views/tab.js` does TWO passes after every `wrapTabBody` invocation: (a) chord-line heuristic via chord-wrap's `isChordLine` + `CHORD_TOKEN_RE` wraps chord-shaped tokens on detected chord lines (covers chord-over-lyric layout — UG-imports with BB stripped, nortabs catalog with `chopro_chord` HTML stripped); (b) regex `\[([A-G][…])\]` wraps ChordPro inline-chord notation (`[G]Hello [Am]world` — common in catalog, ~6000 occurrences). Section markers like `[Intro]` / `[Verse 1]` are excluded — only `[A-G]…`-prefixed contents that pass CHORD_TOKEN_RE match. Pass order matters: chord-line first (plain text), then ChordPro (partially-wrapped — `[X]` can't appear inside our span markup, so no double-wrap). Tier-2 follow-up: `cleanTabBody` also now strips `<pre class="chopro_tab">…</pre>` wrappers (21 catalog tabs had literal HTML rendering as escaped text). The remaining open edge case: chord lines with embedded commentary (`Am (Hold this)` style) fall below the 70%-chord-shaped threshold — extremely rare; revisit with explicit BB-markup parsing if it comes up.

- **Randomized small UG-test-sample generator** (✅ shipped 2026-05-28, `090d87c`): `crawler/sample-ug-export.py`. Stdlib only (argparse + json + random + pathlib). Reads `ug/ug-export-2026-05-16-tommyl.json` by default, samples N tabs (default 15), writes `ug/ug-export-sample-{N}.json`. Flags `-n/--count`, `--source`, `--out`, `--seed`. Preserves the top-level shape so output drops straight into `#/import/ug`. Adds `sampled_from` / `sampled_count` for traceability. Sample output gitignored.

- **Progress UX for long downloads** (✅ shipped 2026-05-28, `9481af7` + `0a5b38b` + `810b225`): (a) `catalog.js`'s `loadCatalog` accepts `opts.onProgress({loaded})` and streams the fetch via `res.body.getReader()`; app.js's loading view shows live `"X.X MB lastet"` (just `loaded` — Content-Length is the compressed ~5 MB while reader emits decompressed ~24 MB, ratio would be misleading). (b) `enrich-ondevice.js` `ensureBase` passes a `monitor(m)` to `LanguageModel.create` that surfaces `downloadprogress` events; the import view shows `"Laster ned Gemini Nano: X.X / Y.Y GB (Z%)"`. The status pill in app.js surfaces the same on background prefetch (kicks off at boot via `prefetchModel()` so the first user-triggered enrichment is instant on machines that haven't downloaded Nano yet).
...
- **Reorder tabs within a songbook** (✅ shipped, `0adf66a`): vanilla ↑/↓ buttons next to each tab in songbook-detail view; `moveTabInSongbook(songbookId, tabId, direction)` in `storage.js` swaps with neighbour. No HTML5 drag-and-drop (bad on touch, music-stand use case is touch-first). Synthetic songbooks (e.g. auto-`ug-import-main`) skip the reorder controls — meaningless when the tab list is derived rather than stored.
...
- **Secondary search fall-through to Ultimate Guitar** (✅ shipped, `f882f3d`): both the zero-results fall-through line AND the always-visible bottom hint in `views/search-bar.js` now offer a `Søk på Ultimate Guitar` link alongside `Søk live på nortabs.net`. Format `ultimate-guitar.com/search.php?search_type=title&value=<encoded>`. New tab, no embedding, no CORS — same pattern as nortabs.
...
- **Ultimate Guitar bookmark import** (userscript shipped + validated on real data; NorTabs-side import UI still to build):
  ...
  - **Acquisition** (✅ implemented, validated on Tommy's real 259-bookmark UG account: 253 OK / 6 failed): a Tampermonkey/Violentmonkey/Greasemonkey userscript runs on `ultimate-guitar.com/user/mytabs*`. It injects a floating "⬇ Eksporter til NorTabs" button, then on click:
    ...
    Source: `userscripts/tabtabtab-ug-exporter.user.js` (moved out of `crawler/` 2026-06-01 — it's a client-side userscript, not crawler tooling). Install in a userscript manager (Tampermonkey, Violentmonkey, or Greasemonkey) — all three accept the same `// ==UserScript==` header format.
